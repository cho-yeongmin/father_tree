...
export function ProtectedTreeFilterSheet({
  ...
  const filteredRegions = useMemo(() => {
    ...
    if (!keyword || tab !== "region") {
      return PROTECTED_TREE_REGIONS;
    }
    return PROTECTED_TREE_REGIONS.filter((item) =>
      ...
      >
        <div className="border-b border-border px-4 pb-3 pt-4">
          ...
          <input
            ...
            placeholder={tab === "region" ? "시·도 검색" : "나무 이름 검색"}
          ...
          {tab === "region" ? (
            filteredRegions.length === 0 ? (
              ...
              filteredRegions.map((item) => {
                const checked = isProtectedRegionSelected(
                  regionSelection,
                  item.name,
                );
